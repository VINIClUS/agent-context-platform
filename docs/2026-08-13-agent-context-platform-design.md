...
- The default `content_addressed` write mode fully verifies and reuses an existing object before
  writing, rejecting metadata conflicts; a newly written object is validated with HEAD.
  `if_none_match` uses `If-None-Match: *` only for S3 implementations that support conditional
  writes, and a losing conditional PUT verifies the existing body before returning. Garage's S3
  API does not provide that conditional-write guarantee, so Garage deployments use the default
